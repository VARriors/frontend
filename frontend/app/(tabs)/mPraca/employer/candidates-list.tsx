...
export default function CandidatesListScreen() {
  ...
  } = useLocalSearchParams<{jobId?: string; jobTitle?: string; employerId?: string}>();
    ...
    const loadCandidates = async () => {
      ...
      try {
        ...
        if (matchesResponse && matchesResponse.matches && matchesResponse.matches.length > 0) {
          const mappedCandidates: CandidateApplication[] = matchesResponse.matches.map(
...
<<<<<<< HEAD
              return {
                ...
                aiMatchFeedback:
                  smartMatch.evaluations
                    ?.map(e => `• ${e.description}: ${e.justification}`)
                    .join('\n') || '',
...
>>>>>>> 10e2b57 (fix issue frontend)
  ...
  const handleCandidatePress = (
    ...
    const resolvedEmployerId =
      typeof employerIdParam === 'string' && employerIdParam.trim()
