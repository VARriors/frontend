...
export default function CandidatesListScreen() {
  ...
  const handleCandidatePress = (id: string, currentStatus: string) => {
    if (currentStatus === 'UNREAD') {
      // 1. Oznacz jako odczytane w lokalnym stanie, żeby kulka zniknęła
      setCandidates(prev => 
        prev.map(c => c.id === id ? { ...c, status: 'VIEWED' } : c)
      );
      
      // 2. TUTAJ STUB: markAsViewed(id) -> Strzał API / Update Zustand
      // Np. dispatch(markApplicationAsViewed(id))
      // Po tym kroku, na urządzeniu Kandydata natychmiast status zmieni się na 'Wyświetlone'.
      console.log(`markAsViewed(${id}) wywołane`);
    }

    // 3. Przekierowanie do CandidateProfileScreen
    // navigation.navigate('CandidateProfile', { candidateId: id });
  };

  const aiMatchList = candidates.filter(c => c.aiMatchScore >= 80).sort((a,b) => b.aiMatchScore - a.aiMatchScore);
  const remainingList = candidates.filter(c => c.aiMatchScore < 80).sort((a,b) => b.aiMatchScore - a.aiMatchScore);
      ...
      >
        <View style={styles.cardHeader}>
          <View style={styles.nameRow}>
            {isUnread && <View style={styles.unreadDot} accessibilityLabel="Nowa aplikacja" />}
            <Text style={[styles.nameText, isUnread && styles.nameTextUnread]}>{item.name}</Text>
          </View>
          <View style={[styles.scoreBox, item.aiMatchScore >= 80 && styles.scoreBoxPremium]}>
            <Text style={[styles.scoreText, item.aiMatchScore >= 80 && styles.scoreTextPremium]}>
        ...
        renderSectionHeader={({ section: { title, isPremium, data } }) => {
          ...
          return (
            <View style={[styles.sectionHeader, isPremium && styles.sectionHeaderPremium]}>
              <Text style={[styles.sectionTitle, isPremium && styles.sectionTitlePremium]}>{title}</Text>
            </View>
          );
        }}
        ListHeaderComponent={
          <View style={styles.pageHeader}>
            ...
            <Text style={styles.pageTitle}>Senior React Native Developer</Text>
            <Text style={styles.pageSubtitle}>Przeskanowane przez system mObywatel. Kandydaci zweryfikowani państwowo.</Text>
          </View>
        }
      />
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  sectionHeader: { marginTop: 16, marginBottom: 12, paddingBottom: 4, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' },
  sectionHeaderPremium: { borderBottomColor: '#FBCFE8' },
  sectionTitle: { fontSize: 18, fontWeight: '700', color: MO_TEXT_SECONDARY },
  sectionTitlePremium: { color: '#BE185D' }, // Premium pink/magenta for AI ✨
  ...
  cardUnread: {
    backgroundColor: '#F0F9FF', // Lekki blues na nowe
    borderColor: '#BAE6FD',
  },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 },
  nameRow: { flexDirection: 'row', alignItems: 'center' },
  unreadDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#0284C7', marginRight: 8, marginTop: 2 },
  nameText: { fontSize: 18, fontWeight: '600', color: MO_TEXT_PRIMARY },
  nameTextUnread: { fontWeight: '800' },
  
  scoreBox: { backgroundColor: '#F3F4F6', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  scoreBoxPremium: { backgroundColor: '#FDF2F8', borderWidth: 1, borderColor: '#FBCFE8' },
  ...
  badge: { backgroundColor: '#ECFDF5', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: '#D1FAE5' },
